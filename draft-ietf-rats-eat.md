...
Note that this is distinct from the already registered "profile" claim in the JWT claims registry.
...
Implementations of EAT as described in this document are far from having any guarantee that they are interoperable because of the large number of options (which are necessary to accommodate the widely varying use cases).
An EAT profile document narrows the specification for a specific use case.
...
The following is a list of EAT, COSE and CBOR options that a profile should address. 
...
An alternate is to allow both definite and indefinite arrays/maps.
...
The Attester will select one of them. Since there is no negotiation, the Verifier should implement all algorithms listed.
...
The ones described this document are only roughly described.
...
The profile should specify whether the token should be a CBOR CWT Tag or not. It is allowed by CWT that the token can either be a tag or not, but this is not recommended.
