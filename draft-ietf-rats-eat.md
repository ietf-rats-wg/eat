...
The following are added as guidance for registering new claims
in the CWT registry.

### Interoperability and Relying Party Orientation

One of the broad goals that CWTs can be processed by
...
that there be some general-purpose verification implementations
...
be applicable to multiple embedded operating systems from multiple
...
are examples of this.  However, CWT and EAT are intended for use in
low security use cases just the same. For example, an app in a mobile
app download system may generate tokens.
...
used by CWT. In most cases it is best to wrap the data or adapt the
encoding to the ones used by CWT so that CWT implementations do not
...
General compliance with CWT allows arbitrary proprietary claims to be
