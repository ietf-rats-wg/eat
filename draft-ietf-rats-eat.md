...
This claim reports the results of a measurement performed on some software.
For example, if the measurements of some software compared correctly to the reference values expected for that software.

This claim may be generated by the verifier and sent to the relying party.
For example, it could be the results of the verifier comparing the contents of the swevidence claim to reference values it has.
...
For example, a TEE might have the ability to measure and SW of the rich OS and may have the reference values for the rich OS.

Multiple results can be reported for multiple measured software objectives.

For each software objective, the following can be reported.

### Scheme
This is the name of the verification system or scheme that peformed the verification.
There is no official registry of schemes or systems.
It may be the name of a commercial product or such.

### Objective
This roughly characterizes the software that was measured.
This corresponds to the attestation target or the submodule.

1 -- All
: Indiciates all the SW has been verified, for example all the SW in the attestation target or the submodule.

2 -- firmware
: Indicates the firmware only

3 -- kernel
: Refers to the most privileged SW, for example the Linux kernel

4: -- privileged
: Refers to the root or system log on.

5: -- system-libs
: Refers to the system libraries that are broadly shared and used by applications and such
...
### Results
This describes the result of the verification.
...
: The verification ran to completion and the result was a failure

4 -- fully-verified
: The verification ran to completion and all checks passed

5 -- partially-verified
: The verification ran to completion. Some checks passed and some didn't.

### Objective Name

This is a text string that describes the the objective.
